...
minServoThrow = 36; %degrees
maxServoThrow = 144; %degrees
maxConvertedAngleValue = 4095;
centerConvertedAngleValue = maxConvertedAngleValue / 2;
servo_Offsets = [50, 50, 0, 0, 0, 0, 0, 0, 0, 0]; %bias needed for each servo to be centered (will not necessarily be centered at centerConvertedAngleValue)
servo_CenterPositions = servo_Offsets + centerConvertedAngleValue; %value at which each servo is centered
Servo_MaxReferences = [2700, 2700, 0, 0, 0, 0, 0, 0, 0, 0]; %input required to give servo 20 degrees of throw above centered position
Servo_MinReferences = servo_CenterPositions - (Servo_MaxReferences - servo_CenterPositions); %input required to give servo 20 degrees of throw below centered position
...
for t = 0:samplePeriod:2*pi/w %increase time in steps of 10ms
    ...
    %calculate servo angle for a given time for the first servo
...
while timeIndexText <= numTimeSteps
    ...
    while servoIndexText <= numServos
        servoAngleMapped(timeIndexText,servoIndexText) = map(servoAngle(timeIndexText,servoIndexText), 70, 110, Servo_MinReferences(servoIndexText), Servo_MaxReferences(servoIndexText));
        servoAngleMapped(timeIndexText,servoIndexText) = round(servoAngleMapped(timeIndexText,servoIndexText));
        fprintf( fid, '%04d', servoAngleMapped(timeIndexText,servoIndexText));
        fprintf( fid, " ");
