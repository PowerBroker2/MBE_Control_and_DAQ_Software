...
#define PC_TO_FISH_DATA_PACKET_SIZE ((NUM_SERVOS * 2) + 5)
...
void getServoData(ifstream& inFile, uint16_t servoPositionArray[]);
void connectToArduinos();
void sendDataPacket(char PCFishDataPacket[], int timeStepIndex, uint16_t servoPositionArray[]);
...
void main()
{
	...
	uint16_t servoPositionArray[NUM_TIME_STEPS * NUM_SERVOS];
...
void getServoData(ifstream& inFile, uint16_t servoPositionArray[])
{
	...
	int numAngleDigits = 4; //number of digits per angle
	int currentDigit = 0; //index for current digit
	...
	while (timeIndex <= NUM_TIME_STEPS)
	{
		//reset servo index
		servoIndex = 1;
		while ((servoIndex <= NUM_SERVOS) && ((arrayIndex + numAngleDigits) < length))
		{
			//reset current digit index
			currentDigit = numAngleDigits;

			//initialize array element to 0;
			servoPositionArray[outputIndex] = 0;

			while (currentDigit)
			{
				servoPositionArray[outputIndex] = servoPositionArray[outputIndex] + (pow(10, (currentDigit-1)) * (textInputBuff[arrayIndex] - '0'));
			...
			//increment buffer index due to whitespace
			arrayIndex = arrayIndex + 1;
...
void sendDataPacket(char PCFishDataPacket[], int timeStepIndex, uint16_t servoPositionArray[])
{
	...
	while (currentServo < NUM_SERVOS)
	{
		//reset i
		i = numBytesPerServo - 1;

		//BIG ENDIAN (MSB FIRST)
		while (i >= 0)
		{
			PCFishDataPacket[packetIndex] = (char)((servoPositionArray[(timeStepIndex * NUM_SERVOS) + currentServo] >> (8 * i)) & 0xFF);

			//increment or decrment counters
